...
public class NewProject extends Activity implements OnDateSetListener, View.OnClickListener{
	...
	protected Dialog onCreateDialog(int dialogId){
		...
		case DIALOG_PRIORITY:
			return new AlertDialog.Builder(NewProject.this)
			.setTitle("Приоритет")
	...
	private void completeForm(Intent intent){
		Cursor c = managedQuery(intent.getData(), new String [] {Columns.COLUMN_NAME_ID,
		...
		if (c != null && c.moveToFirst()) {
			...
			if (colIndex != -1) {
				...
				colIndex = c.getColumnIndex(Columns.PROJECT_COLUMN_PROJECTNAME);
				if (colIndex != -1){
					((EditText)findViewById(R.id.editTextProjectName)).setText(c.getString(colIndex));
					oldValues.put(Columns.PROJECT_COLUMN_PROJECTNAME, c.getString(colIndex));
				}

				colIndex = c.getColumnIndex(Columns.PROJECT_COLUMN_CATEGORY);
				if (colIndex != -1){
					((EditText)findViewById(R.id.editTextCategory)).setText(c.getString(colIndex));
					oldValues.put(Columns.PROJECT_COLUMN_CATEGORY, c.getString(colIndex));
				}

				colIndex = c.getColumnIndex(Columns.PROJECT_COLUMN_STATUS);
				if (colIndex != -1){
					((EditText)findViewById(R.id.editTextStatus)).setText(c.getString(colIndex));
					oldValues.put(Columns.PROJECT_COLUMN_STATUS, c.getString(colIndex));
				...
				colIndex = c.getColumnIndex(Columns.PROJECT_COLUMN_PRIORITY);
				if (colIndex != -1){
					...
					oldValues.put(Columns.PROJECT_COLUMN_PRIORITY, ((EditText)findViewById(R.id.editTextPriority)).getText().toString());
				}

				colIndex = c.getColumnIndex(Columns.PROJECT_COLUMN_PROJECTDUEDATE);
				if (colIndex != -1){
					((EditText)findViewById(R.id.editDate)).setText(
							(c.getString(colIndex)).substring(6, 8) + "-" + 
									(c.getString(colIndex)).substring(4, 6) + "-" +  (c.getString(colIndex)).substring(0, 4));
					oldValues.put(Columns.PROJECT_COLUMN_PROJECTDUEDATE, c.getString(colIndex));
				...
				if (colIndex != -1) {
					...
					mCustOldId = mCustId;
					c = getContentResolver().query(Columns.CUST_INV_URI, new String[] {Columns.COLUMN_NAME_ID, Columns.COLUMN_FIRSTNAME, Columns.COLUMN_MIDDLENAME,
							Columns.COLUMN_LASTNAME}, Columns.COLUMN_NAME_ID + " = \"" + mCustId  + "\"", 
							null, null);
					if (c != null && c.moveToFirst()) {							
			...
			if (c != null && c.moveToFirst()) {
				...
				if (colIndex != -1) {
					...
					mInvOldId = mInvId;
					c = getContentResolver().query(Columns.CUST_INV_URI, new String[] {Columns.COLUMN_NAME_ID, Columns.COLUMN_FIRSTNAME, Columns.COLUMN_MIDDLENAME,
							Columns.COLUMN_LASTNAME}, Columns.COLUMN_NAME_ID + " = \"" + mInvId  + "\"", 
							null, null);
					if (c != null && c.moveToFirst()) {							
						colIndex = c.getColumnIndex(Columns.COLUMN_FIRSTNAME);
						if (colIndex != -1) {
	...
	public void onClick(View v) {
		...
		case R.id.buttonSave:
			if (((EditText) findViewById(R.id.editTextProjectName)).getText().toString().equals(""))
				showDialog("Введите имя проекта");	
			else {
				if (mIsNew)
					saveToDB();
				else if (isChange())
	...
	private void callList(boolean isCustomer){
		Intent intent = new Intent();
		intent.setClass(NewProject.this, CatalogueFragmentActivity.class);	
		...
		int reqCode;
		if (isCustomer) {
			intent.putExtra(CatalogueFragmentActivity.FROM_LIST, CatalogueList.CUSTOMER);
			reqCode = REQUEST_CUSTOMER;
		}
		else {
			intent.putExtra(CatalogueFragmentActivity.FROM_LIST, CatalogueList.INVESTOR);
	...
	private void completeInvestor(Uri uri){
		Cursor c = managedQuery(uri, new String [] {Columns.COLUMN_NAME_ID,
				Columns.COLUMN_NAME_ID,
				Columns.COLUMN_FIRSTNAME,
				Columns.COLUMN_MIDDLENAME,
				Columns.COLUMN_LASTNAME}, null, null, null);
	...
	private boolean isChange(){
		...
		if (oldValues.containsKey(Columns.PROJECT_COLUMN_PROJECTNAME) && !((EditText)findViewById(R.id.editTextProjectName)).getText().toString().equals(oldValues.getAsString(Columns.PROJECT_COLUMN_PROJECTNAME)))
			return true;
		if (oldValues.containsKey(Columns.PROJECT_COLUMN_CATEGORY) && !((EditText)findViewById(R.id.editTextCategory)).getText().toString().equals(oldValues.getAsString(Columns.PROJECT_COLUMN_CATEGORY)))
			return true;
		if (oldValues.containsKey(Columns.PROJECT_COLUMN_STATUS) && !((EditText)findViewById(R.id.editTextStatus)).getText().toString().equals(oldValues.getAsString(Columns.PROJECT_COLUMN_STATUS)))
			return true;
		if (oldValues.containsKey(Columns.PROJECT_COLUMN_PRIORITY) && !((EditText)findViewById(R.id.editTextPriority)).getText().toString().equals(oldValues.getAsString(Columns.PROJECT_COLUMN_PRIORITY)))
			return true;
		if (oldValues.containsKey(Columns.PROJECT_COLUMN_PROJECTDUEDATE) && !((EditText)findViewById(R.id.editDate)).getText().toString().equals(oldValues.getAsString(Columns.PROJECT_COLUMN_PROJECTDUEDATE)))
			return true;
	...
	private void saveToDB(){
		...
		cv.put(Columns.PROJECT_COLUMN_PROJECTNAME, ((EditText) findViewById(R.id.editTextProjectName)).getText().toString());
		cv.put(Columns.PROJECT_COLUMN_CATEGORY, ((EditText) findViewById(R.id.editTextCategory)).getText().toString());
		cv.put(Columns.PROJECT_COLUMN_STATUS, ((EditText) findViewById(R.id.editTextStatus)).getText().toString());
			cv.put(Columns.PROJECT_COLUMN_PROJECTDUEDATE, mDate);
		cv.put(Columns.PROJECT_INVESTOR_ID, mInvId);
		cv.put(Columns.PROJECT_CUSTOMER_ID, mCustId);
		cv.put(Columns.PROJECT_COLUMN_PRIORITY, mPriority);

		if (mIsNew){
			Uri uri = getContentResolver().insert(Columns.PROJECT_URI, cv);
			if (uri == null) 
				Log.e("HLAG", "Failed to insert into " + Columns.PROJECT_URI);	
		} else 
			getContentResolver().update(ContentUris.withAppendedId(Columns.PROJECT_URI, mId), cv, null, null);
